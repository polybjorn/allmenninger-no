---
title: "Allmenninger – LR Mathisen"
description: "Advisory services and project management for sustainable transition. Ecological restoration, climate adaptation and outreach."
machineTranslated: true
nav:
  - { href: "#tenester", label: "Services" }
  - { href: "#om", label: "About" }
  - { href: "#prosjekt", label: "Projects" }
  - { href: "#galleri", label: "Gallery" }
  - { href: "#kontakt", label: "Contact" }
hero:
  eyebrow: "Environment · Climate · Outfield land"
  heading: "Advisory services and project management for sustainable transition"
  lead: "Allmenninger – LR Mathisen helps municipalities, businesses and local communities with ecological restoration, climate adaptation and outreach – built on more than 20 years of experience in the public management of nature and the environment."
  caption: "Controlled heather burning as part of outfield land management"
  image:
    src: "hero-lyngbrenning.jpg"
    alt: "Controlled burning of coastal heathland by the sea, with smoke rising from the managed area"
  actions:
    - { label: "See the services", href: "#tenester" }
    - { label: "Get in touch", href: "#kontakt" }
services:
  kicker: "What I deliver"
  heading: "Services"
  intro: "Suggested packages, adapted to municipalities, businesses and collaborative projects – all of it open to discussion if you would like."
  items:
    - num: "01"
      title: "Habitat management plans for ecological restoration and fire risk"
      lead: "Practical plans that reduce fire risk on outfield land and restore threatened habitat types such as coastal heathland."
      points:
        - "Mapping habitat management needs against fire risk maps"
        - "Prioritising vulnerable areas"
        - "Cooperation agreements with landowners and the municipality"
      image:
        src: "teneste-01-skjotselslag-brenning.jpg"
        alt: "A heathland management group carrying out controlled burning as part of ecological restoration and fire prevention"
    - num: "02"
      title: "Outreach assignments on practical climate adaptation"
      lead: "Outreach, dialogue workshops and facilitation that make climate adaptation concrete and locally grounded."
      points:
        - "Dialogue workshops and climate hackathons"
        - "Talks for professional communities, elected representatives and residents"
        - "Facilitating co-creation between research and public administration"
    - num: "03"
      title: "Teaching programmes on food and sustainability"
      lead: "Practical learning programmes that tie food production, biodiversity and sustainability together for schools and local communities."
      points:
        - "School garden and growing programmes"
        - "Practical learning about local food production"
        - "Linking diet, biodiversity and climate"
      image:
        src: "teneste-03-gronsakhage.jpg"
        alt: "Wellington boots and a grubbing hoe in the soil of a vegetable garden, as part of practical teaching in food and sustainability"
about:
  kicker: "Who I am"
  heading: "About Allmenninger"
  image:
    src: "om-lene-i-felt.jpg"
    alt: "Lene Røkke Mathisen in the field during management work, with a controlled fire and the management group at work behind her"
  facts:
    - title: "Managing director"
      body: "Lene Røkke Mathisen, MSc Water Resources and Coastal Management (University of Bergen), BSc Environmental Science (University of Leeds)."
    - title: "Base"
      body: "Vibrandsøy, Norway. Sole proprietorship, founded in 2025."
    - title: "Background"
      body: "20 years as an adviser and coordinator in municipal and state administration of climate and the environment, including Haugesund municipality, the Norwegian Coastal Administration and the Norwegian Agriculture Agency."
projects:
  kicker: "Selected assignments"
  heading: "Projects"
  intro: "A selection of projects and assignments from recent years."
  items:
    - year: "2025–2026"
      title: "Success factors and barriers in research collaboration: developing and testing the EUReKKa recipe"
      body: "A study commissioned by KS, the Norwegian Association of Local and Regional Authorities, on what drives successful co-creation between the municipal sector and research institutions in developing larger cross-sectoral research applications."
    - year: "2024–2025"
      title: "Coastal heathland management in Haugesund municipality"
      body: "Project coordinator – coordination between municipal agencies, the County Governor and private landowners and farmers on funding and carrying out management work."
    - year: "2024"
      title: "Klimathon Haugesund"
      body: "Municipal representative and co-organiser. Responsible for local, practical tasks, as well as a dedicated day on coastal heathland management and climate adaptation at Ryvarden lighthouse."
    - year: "2021–2026"
      title: "The sea trout project"
      body: "Project manager – mapping, restoration and habitat improvement in sea trout watercourses in Haugesund."
gallery:
  kicker: "From everyday work"
  heading: "Gallery"
  intro: "Glimpses of outfield land, coastal landscape and fieldwork around Vibrandsøy and Haugesund."
  items:
    - image:
        src: "galleri-solnedgang-vibrandsoy.jpg"
        alt: "Sunset over the sea at Vibrandsøy, with a mirror-still water surface"
      caption: "Sunset over the sea"
      tall: true
    - image:
        src: "galleri-fyr-kveldslys.jpg"
        alt: "A lighthouse in evening light at sunset on a coastal island"
      caption: "Evening mood by the lighthouse"
    - image:
        src: "galleri-fjoresona.jpg"
        alt: "Clear water with seaweed and stones in the intertidal zone"
      caption: "Clear water in the intertidal zone"
    - image:
        src: "galleri-born-brygge.jpg"
        alt: "Children on a jetty watching a gull in flight over the water"
      caption: "Children and birdlife by the jetty"
      tall: true
    - image:
        src: "galleri-sau-beite.jpg"
        alt: "Sheep grazing in a green coastal landscape by the sea in the rain"
      caption: "Sheep grazing in the coastal landscape"
skipLink: "Skip to content"
footer: "Allmenninger – LR Mathisen · Vibrandsøy, Norway"
contact:
  heading: "Have a project in mind?"
  body: "Get in touch for an informal chat about habitat management, climate adaptation or outreach assignments."
  email: "lene@allmenninger.no"
---

My name is Lene Røkke Mathisen, and I live on Vibrandsøy, with the sea, the sheep and the heathland as my nearest neighbours. That shapes how I work: I am drawn to projects that grow out of local conditions, where people know the place and its nature well, and where people and nature get to work together rather than against one another.

I am an environmental scientist by training, with a master's degree in Water Resources and Coastal Management from the University of Bergen and a bachelor's degree in Environmental Science from the University of Leeds. For the past 20 years I have worked as an adviser and coordinator on climate and environmental issues, managing projects large and small in mapping, analysis and physical measures.

I have solid experience with outreach assignments aimed at a range of audiences, and with running events. In 2025 I founded Allmenninger to work more deliberately on projects in sustainable transition – through outreach, the facilitation of dialogue workshops and courses, and advisory work on climate adaptation at the local level.

Previously I have worked as environmental adviser and agricultural officer in Haugesund municipality, senior adviser for planning and management at the Norwegian Coastal Administration West, adviser at the Norwegian Agriculture Agency, and adviser at Forsvarsbygg Futura.
